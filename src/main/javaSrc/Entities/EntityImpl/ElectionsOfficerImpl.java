...
public class ElectionsOfficerImpl extends UserImpl implements ElectionsOfficer{
    ...
    public String getRestoreString() throws EVException {
        ...
        String restoreStr = "select Elections_Officer_ID, First_Name, Last_Name, Username, User_Password, Email_Address, Address, City, State, Zip from ElectionsOfficer";
    ...
    public PreparedStatement insertStoreData(PreparedStatement stmt) throws EVException, SQLException {
        ...
        if( getFirstName() != null )
            ...
            throw new EVException( "ElectionsOfficerManager.save: can't save a  First Name undefined" );

        if( getLastName() != null )
            stmt.setString( 2, getLastName() );
        else
            throw new EVException( "ElectionsOfficerManager.save: can't save a  Last Name undefined" );

        if( getUserName() != null )
            stmt.setString( 3, getUserName() );
        else
            throw new EVException( "ElectionsOfficerManager.save: can't save a  Username undefined" );

        if( getUserPassword() != null )
            stmt.setString( 4, getUserPassword() );
        else
            throw new EVException( "ElectionsOfficerManager.save: can't save a  Password undefined" );
